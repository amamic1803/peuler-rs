...
fn main() {
    let argv = command!()
        .arg(Arg::new("problem")
            ...
            .required_unless_present_any(["list", "solutions"])
        ...
        .arg(Arg::new("list")
            ...
            .required(false)
            .conflicts_with_all(["problem", "solutions"])
        )
        .arg(Arg::new("solutions")
            .short('s')
            .long("solutions")
            .action(ArgAction::SetTrue)
            .help("Calculate the solutions for all Project Euler problems")
            .required(false)
            .conflicts_with_all(["problem", "list"])
        )
        .get_matches();

    let list_flag: bool = argv.get_flag("list");
    let solutions_flag: bool = argv.get_flag("solutions");
    let problem_id = if list_flag || solutions_flag { 0 } else { *argv.get_one::<u16>("problem").unwrap() };
