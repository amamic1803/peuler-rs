...
fn solve() -> String {
    ...
    for numerator in 10..100 {
        for denominator in (numerator + 1)..100 {
            let digits_numerator: [u64; 2] = digits(numerator).collect::<Vec<u64>>().try_into().unwrap();
            let digits_denominator: [u64; 2] = digits(denominator).collect::<Vec<u64>>().try_into().unwrap();

            'outer: for m in 0..2 {
                for n in 0..2 {
                    if (digits_numerator[m] == digits_denominator[n]) && (digits_numerator[m] != 0) {
                        let lowest_terms = lowest_common_terms([digits_numerator[(m + 1) % 2], digits_denominator[(n + 1) % 2]]);
