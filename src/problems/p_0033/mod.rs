...
fn solve() -> String {
    ...
    for numerator in 10..100 {
        for denominator in (numerator + 1)..100 {
            ...
            let digits_numerator = [digits_numerator_iter.next().unwrap(), digits_numerator_iter.next().unwrap()];
            let digits_denominator = [digits_denominator_iter.next().unwrap(), digits_denominator_iter.next().unwrap()];
