...
fn solve() -> String {
    ...
    let mut d_t1: u64 = prime_factors(n).into_iter().map(|fact_info| fact_info[1] + 1).product();
    let mut d_t2: u64 = prime_factors((n + 1) / 2).into_iter().map(|fact_info| fact_info[1] + 1).product();

    while d_t1 * d_t2 <= 500 {
        n += 1;
        d_t1 = d_t2;
        if n % 2 == 0 {
            d_t2 = prime_factors(n + 1).into_iter().map(|fact_info| fact_info[1] + 1).product()
        } else {
            d_t2 = prime_factors((n + 1) / 2).into_iter().map(|fact_info| fact_info[1] + 1).product()
        }
    }
